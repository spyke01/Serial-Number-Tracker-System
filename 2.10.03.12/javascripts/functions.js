...
function ajaxShowHideSliderWithImg(itemID) {
	// IE is horrendous about using objects like below so we are unfortunately forced to use the fetchItem function instead of the objects
	obj = fetchItem('slideDiv' + itemID);
	img = fetchItem('slideImg' + itemID);
	...
	if (!obj) {
		...
		if (img) {
			// hide the clicky image if there is one
			fetchItem('slideImg' + itemID).style.display = 'none';
		}
		return false;
	}
	else {
		if (fetchItem('slideStatus' + itemID).value == '0') {
			new Effect.SlideDown('slideDiv' + itemID);
			//status.value = '1';
			fetchItem('slideStatus' + itemID).value = '1';
			if (img) {
				img_re = new RegExp("_collapsed\\.jpg$");
				fetchItem('slideImg' + itemID).src = img.src.replace(img_re, '.jpg');
			}
		}
		else {
			new Effect.SlideUp('slideDiv' + itemID);
			//status.value = '0';
			fetchItem('slideStatus' + itemID).value = '0';
			if (img) {
				img_re = new RegExp("_collapsed\\.jpg$");
				fetchItem('slideImg' + itemID).src = img.src.replace(img_re, '_collapsed.jpg');
			}
		}
	}
	return false;
